...
Each stage's app lives under `mvkit/examples/guide/`. From the repo root, inside
the coco-dev environment:

```sh
make                 # one-time: fetch + build cmoc_os9 and install MVKit
make mvkit-guide     # build every mvkit/examples/guide/NN-* app
```
...
```sh
make -C mvkit/examples/guide/01-hello          # -> build/hello.os9
make -C mvkit/examples/guide/01-hello run      # launch it in MAME (on the host)
```
